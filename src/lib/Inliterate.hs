...
import Language.Haskell.Exts (ImportDecl (..), Module (Module), ModuleName (ModuleName), ParseResult (ParseFailed, ParseOk), SrcSpanInfo, parseFileContents, prettyPrint)
...
parseCodeInfo = Set.fromList . map parse1 . T.words
  where
...
genHaskell doc =
  let cbs = codeBlocks doc
      ...
      doBody =
        concatMap printDoBody (filter inDoBody cbs)
